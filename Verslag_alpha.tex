...
The planned form of concurerency implementation consisted of parallelising functions, which one could join on, and implementing simple semaphore-like reentrant locks. Due to letting go of the functions functionality, concurrency fell off of its basis, which meant implementing it had to be done via an entirely new model like fork-join or simply splitting the different parts up. This proved, with respect to the little time left, impossible to implement without losing absolutely mandatory aspects of the project.
